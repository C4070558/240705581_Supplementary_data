...
plotDensities(z_score, 
   ...
   by TMPRSS2-ERG Fusion Status in mPCa",
...
df <- results %>%
  ...
  mutate(
    ...
    LabelERG = ifelse(GeneSymbol == "ERG", TRUE, FALSE)
  )
...
ggplot(df, aes(x = logFC, y = -log10(adj.P.Val))) +
  ...
  scale_fill_manual(
    name = "EMT-Related Gene Significance", 
    values = c(
      "Upregulated" = "firebrick",
      "Downregulated" = "blue",
      "Not Significant" = "gray"
  ...
  labs(
    title = "Differential Gene Expression of EMT related genes in 
       TMPRSS2-ERG fusion Metastatic Prostate Adenocarcinoma ",
    x = "Log2 Fold Change",
...
ggplot(df_MA, aes(x = AveExpr, y = logFC)) +
  ...
  labs(
    title = "TMPRSS2-ERG Fusion Differentially Expressed Genes in Metastatic Prostate Adenocarcinoma",
